	...
	<div class ="container-fluid">
		...
		<form action="#" id="form_process" method="POST">
			...
			<div class="form-group">
				<label>Code</label>
				<input type="text" placeholder="Code barang" class="form-control" name="code" id="code" value="{{ $code }}">
			</div>
			<div class="form-group">
				<label>Barang</label>
				<input type="text" placeholder="Nama Barang" class="form-control" name="barang" id="barang" value="{{ $barang }}">
			</div>
		</form>
...
@push('scripts')
	<script type="text/javascript">
		$("#btn-save").click(function(){
			...
			if ($("#id").val().length == 0) {
				url = "{{ URL('/') }}/api/v1/barang/create";
			}else{
				url = "{{ URL('/') }}/api/v1/barang/update";
			}
